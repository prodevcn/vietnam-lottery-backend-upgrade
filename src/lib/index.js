...
exports.createMegaLottoNumbers = () => {
  ...
  for (let index = 0; index < 6; index++) {
    _target[labels[index]] =
      numbers[index] > 9 ? numbers[index].toString() : '0' + numbers[index];
  }
  return _target;
};
...
const getNorthernResultsFromConfirmURL = async () => {
  const data = (await axios.get(conf.resultUrlNorthern)).data;
  ...
  const jackpot =
    tableRows[0].getElementsByTagName('td')[1].childNodes[0].rawText;
  ...
  lottoPairs.push(
    tableRows[1]
      ...
      .childNodes[0].rawText.replace(/\s/g, '')
  );

  const firstPrize =
    tableRows[2].getElementsByTagName('td')[1].childNodes[0].rawText;
  lottoPairs.push(
    tableRows[2]
      .getElementsByTagName('td')[3]
      .childNodes[0].rawText.replace(/\s/g, '')
  ...
  const secondPrize =
    elements[0].childNodes[0].rawText + '-' + elements[1].childNodes[0].rawText;

  elements = tableRows[4]
    .getElementsByTagName('td')[1]
    .getElementsByTagName('span');
  lottoPairs.push(
    tableRows[4]
...
const getNorthernResultsFromURL1 = async () => {
  const data = (await axios.get(`${conf.resultUrl_1}/getkqxs/mien-bac.js`))
    .data;
  const phrase = parse.parse(data);
  const tbody = phrase.getElementsByTagName('tbody')[0];
  const tableRows = tbody.getElementsByTagName('tr');
  const jackpot = tableRows[1]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  const firstPrize = tableRows[2]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  const secondPrize = tableRows[3]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  const thirdPrize = tableRows[4]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  const fourthPrize = tableRows[5]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  const fifthPrize = tableRows[6]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  const sixthPrize = tableRows[7]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  const seventhPrize = tableRows[8]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  return {
    redAward: jackpot,
...
const getSouthernResultFromConfirmURL = async () => {
  const data = (await axios.get(conf.resultUrlSouthern)).data;
  const phrase = parse.parse(data);
  const tbody = phrase
  ...
  let eighthPrize = tableRows[0]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let seventhPrize = tableRows[1]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let sixthPrize = tableRows[2]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let fifthPrize = tableRows[3]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let fourthPrize = tableRows[4]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let thirdPrize = tableRows[5]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let secondePrize = tableRows[6]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let firstPrize = tableRows[7]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
  let jackpot = tableRows[8]
    .getElementsByTagName('td')[1]
    .childNodes[0].rawText.replace(/\s/g, '');
...
exports.getNorthernLottoNumbers = async () => {
  const northernResult1 = await getNorthernResultsFromConfirmURL();
  const northernResult2 = await getNorthernResultsFromURL1();
...
exports.getSouthernHochiminhLottoNumbers = async () => {
  const southernResult = await getSouthernResultFromConfirmURL();
  return southernResult;
};

exports.getCentralQuangNamLottoNumbers = async () => {
  // return numbers;
  const numbers = await getLottoNumbersFromURL1(
    `${conf.resultUrl_1}/getkqxs/quang-nam.js`
  );
  return numbers;
};
